...
class SassExpandedRenderer extends SassCompactRenderer {
    ...
    protected $previousNodeWasComment = false;
	...
	public function renderComment($node) {
		...
		if ( count($lines) == 1 && substr($lines[0], 0, 1) == '*' && substr($lines[0], -1) == '*' ) {
		    // Munger comment - leave
		    return '/*'.$lines[0].'*/';
		}
		return "$indent/*\n$indent * ".join("\n$indent * ", $lines)."\n$indent */".(empty($indent)?"\n":'');
	}
	...
	public function renderProperties($node, $properties) {
        ...
        $out = '';
        $munger_comment = false;
        foreach( $properties as $property ) {
            $join = ( substr($property, 0, 3) == '/**' && substr($property, -3) == '**/' )
            ? '' : "\n";
            $out .= $join . $indent . $property;
        }
		return $out;
	}
}
